...
class VarAttention(nn.Module):
    ...
    def forward(self, x, einops_from, einops_to, **einops_dims):
        ...
        # q *= self.scale
        q_scaled = q * self.scale  # this is the updated line


        # splice out CLS token at index 1
        (cls_q, q_), (cls_k, k_), (cls_v, v_) = map(lambda t: (t[:, 0:1], t[:, 1:]), (q_scaled, k, v))
...
class SpaceTimeTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0., hybrid_backbone=None, norm_layer=None, add_dim=True,
                 num_frames=8, time_init='rand', attention_style='frozen-in-time', permute_encoder_hidden=None):
        """
        Args:
        ...
        self.num_frames = num_frames
        self.embed_dim = embed_dim
        self.embed_dim2 = 12545
        norm_layer = norm_layer or partial(nn.LayerNorm, eps=1e-6)
        self.add_dim = add_dim
        self.permute_encoder_hidden = permute_encoder_hidden
        ...
        # Classifier head
        self.head = nn.Linear(self.embed_dim2, num_classes) if num_classes > 0 else nn.Identity()
    ...
    def forward(self, x):
        ...
        if self.permute_encoder_hidden:
            x = self.head(x.permute(self.permute_encoder_hidden))        
        else:
            x = self.head(x)
        return x
